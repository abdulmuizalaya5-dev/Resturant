...
const BREVO_FROM_EMAIL = process.env.BREVO_FROM_EMAIL || 'noreply@cyanreserve.com';
const BREVO_FROM_NAME = process.env.BREVO_FROM_NAME || 'CyanReserve';
...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      <p style="margin-top:6px"><a href="${NEXT_PUBLIC_APP_URL}">Visit CyanReserve</a> · <a href="${NEXT_PUBLIC_APP_URL}/restaurants">Explore Restaurants</a></p>
...
async function sendWelcomeEmail(user) {
  ...
  const bodyHtml = `
    ...
    <a href="${NEXT_PUBLIC_APP_URL}/restaurants" class="btn">Explore Restaurants →</a>
  `;
  ...
  return sendBrevoEmail({
    ...
    textContent: `Welcome to CyanReserve, ${user.name}! Your ${rolePretty} account is ready. Visit: ${NEXT_PUBLIC_APP_URL}/restaurants`,
  });
}
...
async function sendBookingConfirmationEmail(booking) {
  ...
  const bodyHtml = `
    ...
    <a href="${NEXT_PUBLIC_APP_URL}/dashboard/customer" class="btn">View My Bookings →</a>
  `;
  ...
  return sendBrevoEmail({
    ...
    htmlContent: emailWrapper(bodyHtml),
    textContent: `Hi ${firstName}! Your table at ${booking.restaurantName} on ${booking.date} at ${booking.time} for ${booking.guests} guests has been received. Booking ref: ${booking.id}. View at: ${NEXT_PUBLIC_APP_URL}/dashboard/customer`,
  });
}
...
async function sendLoginAlertEmail(user) {
  ...
  const bodyHtml = `
    ...
    <a href="${NEXT_PUBLIC_APP_URL}/dashboard/customer" class="btn">Go to Dashboard →</a>
  `;
